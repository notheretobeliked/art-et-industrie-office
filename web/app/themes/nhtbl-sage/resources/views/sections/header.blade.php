          ...
          <nav id="language-switcher">
            <ul class="list-none flex flex-row gap-2">
              @foreach ($languages as $language)
                <li
                  class="font-serif font-sm uppercase pb-1 {{ $language['current_lang'] ? 'border-b border-black dark:border-white' : '' }}">
                  <a href="{{ $language['url'] }}" class="text-black dark:text-white hover:text-stroke-0">
                    {{ $language['slug'] }}
                  </a>
                </li>
              @endforeach
  ...
  @if (has_nav_menu('primary_navigation'))
    <nav class="nav-primary flex flex-col fixed top-0 z-10 bg-white text-black w-full h-screen" x-show="showMenu"
      ...
      <div class="w-full h-screen relative">
        <img src="@asset('images/degrade.svg')" :class="{ 'hidden': $store.quality.qualitySwitch === 'webp-bw' }"
