<div class="relative w-full max-w-screen @if ($size == 'large') h-[80vh] @else h-96 @endif">
  ...
  @if (in_array($slug, ['resonance', 'all', 'triennale', 'oeuvres-publics'], true))
    <div @pin.window="placedata = $event.detail.features[0].properties;" x-data="{ placedata: null }"
      ...
      <div class="flex items-center justify-between">
        ...
        <button type="button" class="z-50 cursor-pointer"
          @click="document.getElementById('feature-info').classList.remove('right-0')">
      ...
      <figure x-show="placedata.image">
        <picture class="block object-cover" x-data="{
            qualitySwitch: $store.utils.getCookie('qualitySwitch') || 'webp-low',
            basePath: '/app/uploads' + (placedata.image ? placedata.image.subdir : '') + '/',
            imageSources: {
                'webp': placedata.image ? placedata.image.other_formats.large.webp : '',
                'webp-low': placedata.image ? placedata.image.other_formats.large.webp - low : '',
                'webp-bw': placedata.image ? placedata.image.other_formats.large.webp - bw : ''
            }
        }">
          <source x-data="console.log(basePath)" :srcset="basePath + qualitySwitch" media="(min-width: 0px)" alt=" ">
          <img :width="placedata.image ? placedata.image.width : 'auto'"
            :height="placedata.image ? placedata.image.height : 'auto'" class="w-full h-auto"
            :src="placedata.image ? placedata.image.src[0] : ''" :alt="placedata.image ? placedata.image.alt : ''" />
        </picture>
      </figure>
      <div x-html="placedata.acces ? placedata.acces : ''" class="pb-3 font-serif text-base"></div>
